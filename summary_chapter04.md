...
- **뜻**: 여러 개의 자원(객체, 연결, 스레드 등)을 **모아 놓은 저장소/집합**. 필요할 때 꺼내 쓰고 다 쓰면 다시 돌려주는 구조.
- **특징**
    - **재사용성**: 매번 새로 만드는 대신 기존 자원을 재활용 → 성능 최적화.
    - **관리 용이**: 자원 낭비를 막고, 동시에 사용 가능한 개수를 제어 가능.
    - **예시**
        - **DB Connection Pool**: 매번 DB 연결을 새로 만드는 대신, 미리 만들어둔 연결을 빌려서 사용.
        - **Thread Pool**: 요청마다 새 스레드를 생성하지 않고, 미리 만들어둔 스레드 집합에서 가져와 처리.
